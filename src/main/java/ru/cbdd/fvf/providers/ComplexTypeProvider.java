package ru.cbdd.fvf.providers;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;
import org.sql2o.Connection;
import org.sql2o.Sql2o;
import ru.cbdd.fvf.entitys.ComplexType;
import ru.cbdd.fvf.interfaces.iproviders.IComplexTypeProvider;

@Component
public class ComplexTypeProvider implements IComplexTypeProvider {
    private final Sql2o sql2o;

    private static final String SELECT_TYPE_BY_ID = "select * from complex_type where id = :id";

    public ComplexTypeProvider(@Autowired Sql2o sql2o) {
        this.sql2o = sql2o;
    }

    @Override
    public ComplexType findById(Long id) {
        try (Connection connection = sql2o.open()) {
            return connection.createQuery(SELECT_TYPE_BY_ID, false)
                    .addParameter("id", id)
                    .setColumnMappings(ComplexType.COLUMN_MAPPINGS)
                    .executeAndFetchFirst(ComplexType.class);
        }
    }
}
